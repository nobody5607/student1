...
<div class="stock-brand-form">
	...
	<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        <h4 class="modal-title" id="itemModalLabel"><i class="fa fa-table"></i> Stock Brand</h4>
    </div>
    ...
    <div class="modal-body">
        ...
        ?>
	<?= $form->field($model, 'type')->dropDownList($items,['prompt'=> Yii::t('product','Select Brand type')]) ?>

	<?= $form->field($model, 'name')->textInput(['maxlength' => true]) ?>
        <?= $form->field($model, 'brand_id')->dropDownList($mainBrands,['prompt'=> Yii::t('product','Select Brand')]) ?>
        ...
        success:function(result){
           ...
           if(result.status == 'success') {
            ...
            if(result.action == 'create') {
                //$(\$form).trigger('reset');
                $(document).find('#modal-stock-brand').modal('hide');
